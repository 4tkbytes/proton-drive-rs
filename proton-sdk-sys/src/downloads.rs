...
pub mod raw {
    use crate::{data::{AsyncCallback, AsyncCallbackWithProgress, ByteArray}, drive::DriveClientHandle, ProtonSDKLib};
    ...
    pub fn downloader_create(
        ...
        unsafe {
            ...
            let create_downloader_fn: libloading::Symbol<unsafe extern "C" fn(
                isize,
                ByteArray,
                AsyncCallback,
            ) -> i32> = sdk.sdk_library.get(b"downloader_create")?;
            
            let result = create_downloader_fn(
                client_handle.raw(),
                request,
                callback,
            );
    ...
    pub fn downloader_download_file(
        ...
        unsafe {
            ...
            let download_file_fn: libloading::Symbol<unsafe extern "C" fn(
                isize,
                ByteArray,
                AsyncCallbackWithProgress,
            ) -> i32> = sdk.sdk_library.get(b"downloader_download_file")?;
            
            let result = download_file_fn(
                downloader_handle.raw(),
                request,
                callback,
            );
