use std::{env, io::{Read, Result, Write}, path::PathBuf};
use anyhow::*;
...
fn download_libs() -> anyhow::Result<()> {
    ...
    if let Ok(custom_path) = env::var("PROTON_SDK_NATIVE_PATH") {
        ...
        if custom_lib_dir.exists() {
            println!("cargo:warning=Using custom native library path: {}", custom_lib_dir.display());
            ...
            println!("cargo:warning=Custom path {} does not exist, falling back to download", custom_lib_dir.display());
        }
    }
    ...
    if arch == "arm64" && (platform == "windows" || platform == "linux") {
        ...
        return Err(anyhow!("ARM64 builds for {} require manual compilation", platform));
    }

    let runtime_id = format!("{}-{}", platform, arch);
    let version = env::var("PROTON_SDK_VERSION").unwrap_or_else(|_| "v1.0.0".to_string());
    
    println!("cargo:warning=Downloading native libraries for {}", runtime_id);
    ...
    println!("cargo:warning=Successfully set up native libraries for {}", runtime_id);
...
fn copy_libs_to_target(source_dir: &PathBuf, platform: &str) -> anyhow::Result<()> {
    ...
    println!("cargo:warning=Copying native libraries to executable directory: {}", exe_dir.display());
        ...
        for entry in std::fs::read_dir(source_dir)? {
            ...
            if file_path.is_file() {
                ...
                if is_library {
                    ...
                    println!("cargo:warning=Copied {} to {}", file_name.to_string_lossy(), dest_path.display());
                }
            }
        }
    }
...
fn download_and_extract(url: &str, dest_dir: &PathBuf, file_extension: &str) -> anyhow::Result<()> {
    ...
    let temp_file = dest_dir.parent().unwrap().join(format!("temp_download.{}", file_extension));
    ...
    println!("cargo:warning=Successfully extracted native libraries to {}", dest_dir.display());
    
    Ok(())
}

fn process_response(response: &[u8], response_str: String, dest_dir: &PathBuf, file_extension: &str) -> anyhow::Result<()> {
...
After build, you'll find the library at: native-libs/{}-arm64/
 ...
 - OPTION 1: Copy manually to executable directory:
   {}
   
 - OPTION 2: Set environment variable and rebuild:
```bash
export PROTON_SDK_NATIVE_PATH=/path/to/your/native-libs/{}-arm64
cargo build
```
...
==============================================================================================
    ", platform, platform, exe_dir.display(), platform);
}
...
fn parse_target(target: &str) -> anyhow::Result<(String, String)> {
    ...
    let (arch, platform) = match parts.as_slice() {
        ...
        ["i686", "pc", "windows", ..] => panic!("Please use a 64 bit operating system or build the library yourself..."),
        
        ["x86_64", "unknown", "linux", ..] => ("x64", "linux"),
        ["aarch64", "unknown", "linux", ..] => ("arm64", "linux"),
        ["i686", "unknown", "linux", ..] => panic!("Please use a 64 bit operating system or build the library yourself..."),
